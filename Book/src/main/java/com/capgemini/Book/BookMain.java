...
import com.capgemini.Book.exceptions.PriceIsNotValidExeption;
...
public class BookMain {
		...
		if (books.size() < 1) {
			...
			do {
				try {
					...
					price = sc.nextDouble();
					if (price <= 0) {
						throw new PriceIsNotValidExeption("Price is not valid");
					}
				} catch (PriceIsNotValidExeption e) {
					System.out.println("Error Message : " + e.getMessage());
				}
			} while (price <= 0);
	...
	public static void read() {
		try {
			if (books.size() < 1) {
				throw new ArrayListNullException("ArrayList is null.");
			} else {
				for (int i = 0; i < books.size(); i++) {
	...
	public static void delete() {
		try {
			if (books.size() < 1) {
				...
				try {
					...
					if (temp == 0) {
						throw new BookAlreadyFoundException("Book Not Found!!!");
					} else {
						books.remove(k);
		...
		} catch (ArrayListNullException er) {
